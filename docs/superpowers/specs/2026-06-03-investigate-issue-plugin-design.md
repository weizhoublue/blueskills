...
4. **结论** — **仅一行** `REVIEW_RESULT=issue_true` 或 `REVIEW_RESULT=issue_false`，无任何其他文字
...
阶段 4   issue-writer（draft_all，一次写齐四节含结论）
...
```text
委派 issue-writer(mode=draft_all)    # 四节初稿（含 issue-verdict），无 challenger
round ← 1
while round ≤ 3:
...
12. **结论（R19）**：`issue-verdict.md` 整文件仅一行 `REVIEW_RESULT=issue_true|issue_false`；禁止任何其他文字；选用与前三节一致。
13. Read/Write 中间产物仅 `$ISSUE_TMP/**` + 被分析仓库只读。
14. **机制动机（R18）**：关键机制须可回答 W1–W3（角色、为何采用该手段、失灵如何接到症状）；challenger 以 `major` 检出；详见 [`2026-06-04-investigate-issue-mechanism-motivation-design.md`](2026-06-04-investigate-issue-mechanism-motivation-design.md)。
...
（组装时：`issue-verdict.md` 仅一行 `REVIEW_RESULT=…`，禁止追加说明。）
