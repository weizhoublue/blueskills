...
主 Agent 启动**一个**采集子 Agent（Task），完成如下流程。子 Agent prompt 必须包含：`TMP_DIR`、`debug`、MemPalace **只读**参数、agent-browser 优先原则、困难与统计上报规范。

#### 1.1 趋势榜采集

数据来源 GitHub Trending `https://github.com/trending`  该页面展示了 GitHub 当天的热门项目

- 使用 /usr/sbin/agent-browser 访问页面，提取 `https://github.com/<owner>/<repo>` 格式 URL
- 规范化：去掉尾部 `/`、query、fragment；统一小写 owner/repo 用于去重
- 使用降级工具（非 agent-browser）时，必须记录困难与统计
- **禁止安装 npm i -g agent-browser**
- `debug=true` 时在 `TMP_DIR/collect/` 下落盘采集到的 URL 列表 Markdown 文件。
...
- 有命中 → 移除（已分析过）
- 无命中 → 保留
- `debug=true` 时在 `TMP_DIR/collect/` 下落盘去重后的保留与移除 URL 列表 Markdown 文件。

主 Agent / 采集子 Agent 须维护分类剔除列表 `excluded_urls`（包括 `history_analyzed` 与 `star_insufficient`），以 Markdown 列表格式保存。

- Step 1.2 的 `removed` URL 填入 `history_analyzed`
- `star_insufficient` 在采集阶段保持空，由 Step 2 主 Agent 在收到 `skipped_low_stars` 时追加
- **不含**第 2 步 `failed` 项目

`debug=true` 时可落盘 `excluded_urls` 快照 Markdown 文件（随 Step 2 追加更新）。
