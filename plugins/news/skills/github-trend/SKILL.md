...
| `stage` | 如 `1.1_trending` `1.2_history` `1.3_stars` `2_analyze` `3_mempalace_write` `4_report` |
...
| 来源 | URL | 采集上限 | 说明 |
|------|-----|------|----|
| OSS Insight | `https://ossinsight.io/trending?period=past_24_hours` | 15 | 该页面展示了过去 24 小时内的热门 github 项目 |
| GitHub Trending | `https://github.com/trending` | 15 | 该页面展示了 GitHub 当天的热门项目 |
...
对 kept 列表中每个 URL，/usr/sbin/agent-browser 访问仓库页，读取 star 数。

- star < 5000 → 剔除
- star ≥ 5000 → 保留
- 无法读取 star 时：记录到 `removed` 并注明原因，上报 `[warning]` 困难，不终止整体流程
- `debug=true` 时写入 `TMP_DIR/collect/filtered_stars.json`：
  ```json
  {"kept": [{"url": "...", "stars": 12345}], "removed": ["https://github.com/owner/repo", ...], "kept_count": Z}
  ```

主 Agent / 采集子 Agent 须维护 `excluded_urls`：合并第 1.2 步 `removed` 与第 1.3 步 `removed` 中的全部 URL（去重、保持首次出现顺序）。**不含**第 2 步分析失败的项目。
...
若 `after_stars_count` 为 0，主 Agent 跳至第 4 步输出「今日无新项目」并结束（仍须输出「剔除分析项目」与「执行困难汇总」；**跳过第 3 步 MemPalace 写入**）。
...
最终报告格式使用如下 markdown 格式
  ...
  仅列出第 1 步中被剔除、未进入第 2 步分析的 GitHub 项目 URL（来自 `excluded_urls`）。**禁止**附加原因、说明或分类，仅 URL 列表。

  - https://github.com/<owner>/<repo>
  - https://github.com/<owner>/<repo>

  （`excluded_urls` 为空时写：无）
