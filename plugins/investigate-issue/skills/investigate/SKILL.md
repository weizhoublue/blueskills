---
description: 针对开源项目单个问题做深度分析（自由文本输入）。在目标仓库根目录运行；只读分析、不跑测试；四节报告（含结论 REVIEW_RESULT）最终仅输出到 stdout。编排 issue-scout、code-tracer、business-context-analyst、issue-writer、issue-challenger（四节初稿后整稿评审，最多 3 轮）。
---
...
12. **结论（R19）**：`sections/issue-verdict.md` **整文件仅一行** `REVIEW_RESULT=issue_true` 或 `REVIEW_RESULT=issue_false`；**禁止**任何其他文字。终稿 `## 4. 结论` 下也只输出该行。选用须与前三节分析一致。
...
**`MAX_REVIEW_ROUNDS = 3`**（**整份四节报告**（含结论）合计最多 3 轮深化；**非**每节独立计数）。
...
（须含：**正向须同时满足** + **不触发/表现为正常的情形**（反向）；禁止单一配置键 = 必然触发。可含 `### 未能从代码确认的前提`：inference 场景不得计入「须同时满足」，见 R20。）
...
| 4 | issue-writer | sections/*.md（draft_all 一次写齐四节） |
...
- **问题后果** ← trace + business
- **触发条件** ← trace + scout
- **结论** ← 综合前三节 + `issue-analysis.json`（writer 在 `issue-verdict` 归纳）
