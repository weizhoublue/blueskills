...
**读者**：未读过仓库的工程师。深度 = **多层因果（L1 情境 → L2 后果 → [L3 默认方案不足] → L4 本项目抽象机制 → L5 用户结果）+ 术语首现解释 + refs 对齐**。

**两阶段（每条 scenario / problem）：** 先证据卡（claim + ref + 证明什么）→ 再写 `narrative` / `causal_chain` / `contrast` / `mechanism_at_a_glance` / 可选 `key_mechanisms[]`（禁止无 ref 新主张）。

**八项自检：** 情境、对比、因果链、机制(L4)、用户结果、术语、证据、**机制动机（关键机制 W1+W2）**。

**`principle` 各维度**（`activation_flow` / `processing_stages` / `external_interactions` 等）每条 statement 须含 **动作 + 动机（W2 一句）**，禁止仅列组件名。

**`key_mechanisms`：** Disaggregated、Gateway+EPP、连接复用/长连接等主题建议 ≥1 条；无材料则 `[]`。

**浅 / 深对照（功能级，勿模仿浅例）：**

- 浅：「支持 OpenAI 协议解析。」
- 深：「网关需同时接入 OpenAI 与 vLLM 客户端；若 parser 不能识别 disaggregated 路径，预填与解码请求会落到错误 handler，用户看到 4xx 或空响应（L2）。本项目在 EPP 侧注册多 parser，按路径与 Content-Type 选择（L4，refs: …）。运维可在同一 Ingress 后挂多种后端而无需改客户端 SDK（L5）。」

- `scenarios` ≥ 2 条 NarrativeBlock；`problems_solved` ≥ 2 条。
- 每条含 `causal_chain[]`（至少 L1/L2/L4/L5）、建议 `contrast` + `mechanism_at_a_glance`；`narrative` 由 chain 合成；**字数不作为目标**（结构齐全即可）。
- 每条 `problems_solved[]` 建议含 `addressed_by_principle_dims`：`["activation_flow","processing_stages",...]`（与 `principle` 五维对应）。
- `industry_context_notes` ≤ 2 条；不得把 `industry_context` 放进主列表。
- 每个 `sub_features`：`narrative` 150~300 字，`boundary_with_parent` 必填。
- **产物 1（.md）禁止 markdown 表格（R15）**；用 `###` 与列表。
...
3. **再读代码验证**：按 `code_paths` 与 `evidence_samples` 中 `kind in (cli, api, crd, config, code-comment)` 的项定向读取；**不要无差别遍历**。预算上限：**单次 Read ≤ 200 行；整轮 Read 总数默认 ≤ 35**，主线程 `read_budget: 45` 时 ≤ **45**（优先 CHANGELOG、设计 doc、ADR）；**整轮 Grep 总数 ≤ 15 次**；**Glob ≤ 8 次**，仅用于在 `code_paths` 内定位文件后再 Grep，禁止仓库级全局 Glob。
...
```json
{
  ...
  "scenarios": [
    {
      "title": "≤ 40 字",
      "narrative": "150~400 字，由 causal_chain 合成",
      "contrast": "≤ 80 字",
      ...
      "background": "",
      "terms": [{"term": "...", "glossary": "是什么 + 在本功能中的作用"}],
      "refs": ["..."],
      "key_mechanisms": []
    }
  ],
  "problems_solved": [
    {
      ...
      "causal_chain": [
        {"layer": 1, "statement": "...", "refs": []},
        {"layer": 2, "statement": "...", "refs": []},
        {"layer": 3, "statement": "可选：默认方案为何不够", "refs": []},
        {"layer": 4, "statement": "...", "refs": []},
        {"layer": 5, "statement": "...", "refs": []}
      ],
      "addressed_by_principle_dims": ["processing_stages", "external_interactions"],
      "evidence_tier": "doc_declared",
      "background": "",
      "terms": [],
      "refs": ["..."],
      "key_mechanisms": [{
        "name": "sidecar → prefill 长连接",
        "w1_role": "P/D 部署中 decode 侧路由层对 prefill 的出站转发",
        "w2_why_not_alternative": "复用 TCP，降低握手延迟；相对每请求短连接",
        "w3_when_breaks": "与 prefill keep-alive 不一致时复用失败，转发报错",
        "evidence_tier": "inference",
        "refs": [],
        "uncertainty_note": "未能从代码确认超时数值"
      }]
    }
  ],
  ...
  "principle": {
    "summary": "...",
    "activation_flow": ["..."],
    "processing_stages": ["在 Filter 阶段通过 ext-proc 拉取上下文（动作），以便调度感知 prefix 而非轮询（动机，refs: …）"],
    "state_changes": ["..."],
...
- 逐条处理 `severity ∈ {blocking, major}`：按质询补 **因果层 / causal_chain / contrast / mechanism / key_mechanisms**；`mechanism_motivation` 优先补 W 层；禁止仅加长 narrative；同步更新 `.md`（仍禁表格）。
