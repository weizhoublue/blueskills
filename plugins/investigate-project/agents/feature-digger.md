...
**两阶段（每条 scenario / problem）：** 先证据卡（claim + ref + 证明什么）→ 再写 `narrative` / `causal_chain` / `contrast` / `mechanism_at_a_glance`（禁止无 ref 新主张）。

**七项自检：** 情境、对比、因果链、机制、用户结果、术语、证据。
...
- 逐条处理 `severity ∈ {blocking, major}`：按质询补 **因果层 / causal_chain / contrast / mechanism**、术语、refs/tier；禁止仅加长 narrative；同步更新 `.md`（仍禁表格）。
