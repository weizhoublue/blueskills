...
- 成立：补充证据；将触发条件改为 T1 v2 + T3 v2 合规双块（顶层逻辑条件 + 参考触发场景，结构化出处、无禁止词）；适当修正严重性等级；
- 不成立：删除；
- 证据不足：删除。
...
- 描述模糊但成立 → **补充**证据与触发条件，改成确定性表述；**禁止**仅润色后删除（触发条件须符合 T1 v2 + T3 v2，禁止仅删掉禁止词而不补结构化出处）。
