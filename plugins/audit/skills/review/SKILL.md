...
- **T3 场景具象化**：顶层逻辑条件之后须提供 **参考触发场景** 块（≥1 条）。每条须含：场景来源（`code_synth` / `pr_context` / `hybrid`）、映射逻辑条件编号、软件配置、业务输入、应用层行为、量化观测。**严禁**「大概」「可能」「未来」「某些情况下」及同义含糊语；须具体取值；无法量化写「未能从代码量化」，禁止用含糊句替代。
...
- 缺陷的触发条件：
  ...
  - **参考触发场景**（可评估；T3）：
    - **场景1**（来源：code_synth | pr_context | hybrid）
      - **映射条件**：条件1 + 条件2
      - **软件配置**：`key=value` … refs: path:line 或 PR 引用
...
| 内容 | 写入位置 |
| --- | --- |
| CR/Helm/env/API/CLI 配置与输入 | 顶层逻辑条件 / 软件配置 / 业务输入 |
