...
mkdir -p "$AUDIT_TMP/findings" "$AUDIT_TMP/challenges" "$AUDIT_TMP/peer-challenges"
```
...
```text
1. AUDIT_TMP=$(mktemp -d)；mkdir findings challenges peer-challenges；配置 trap
